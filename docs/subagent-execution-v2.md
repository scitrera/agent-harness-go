# Subagent Execution Envelope v2

Status: implemented for in-process, detached, and opt-in targeted Aether
execution. Revision 1 remains readable only for already-admitted unbound tasks
...
- a deterministic `ahx-v2-*` execution ID and schema revision;
...
4. Resuming a child thread compares the new scope with the latest persisted
   child execution input. Legacy-unbound to bound, bound to unbound, view changes,
   and write expansion require the same explicit authorization.
...
persist input message + exact scope in shared history
        |
        v
admit idempotent task with v2 envelope + scope digest metadata
        |
        v
...
Revision 1 has no `execution_scope` and retains `ahx-v1-*` identity/checkpoint
rules. The revision-2 parser accepts it so queued unbound work survives a rolling
upgrade. A revision-1 envelope carrying a scope is invalid, and new admissions
always use revision 2. An old executor encountering a version-2 payload rejects
the unknown revision rather than silently discarding its view authority.
